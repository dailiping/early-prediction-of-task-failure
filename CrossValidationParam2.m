function [best_param1,best_param2] = CrossValidationParam2...
...
%   Last modified on June 3, 2012.
    ...
    task_num = length(X);
    % performance vector
    perform_mat = zeros(length(param_range1)+length(param_range2),1);
    ...
    for cv_idx = 1: cv_fold
        ...
        %}
        ACC=0;
        for p_idx = 1: length(param_range1)
            for r_idx=1:length(param_range2)
                %调第一个参数
                %[W,~] = obj_func(cv_Xtr, cv_Ytr, param_range(p_idx),rho,  obj_func_opts);
                %调第二个参数
                [W,~] = obj_func(cv_Xtr, cv_Ytr, param_range1(p_idx),param_range2(r_idx),  obj_func_opts);
                acc = perform_mat(p_idx) + eval_func(cv_Yte,cv_Xte, W);%句柄，调用函数
                if acc>ACC
                    ACC=acc;
                    best_param1=param_range1(p_idx);
                    best_param2=param_range2(r_idx);
                end
            end
        end
    end
...
%      end
%      best_param1=param_range1(best_idx);
end